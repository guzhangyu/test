...
public abstract class AbstractQueuedSynchronizer1
    ...
    private void doReleaseShared() {
         ...
         */
        //并且，不像其他地方使用unparkSuccessor那样，我们需要知道重置status的cas动作是否有成功，所以需要再次检查
        for (;;) {
            Node h = head;
...
//                        !compareAndSetWaitStatus(h, 0, Node.PROPAGATE))
                    //如果unparkSuccessor中通知的进程已经修改了head节点，会出现waitStatus为0，要将其改为PROPAGATE，来保证在新节点release之后，传播行为继续
                    continue;                // loop on failed CAS
            }
            if (h == head)                   // loop if head changed
                break;
            //如果unparkSuccessor中通知的进程已经修改了hea的节点，循环继续
        }
    }
     ...
     */
    //设置新的头节点，并检查后继节点是否读锁
    // 如果是，并且propagate>0或者后继节点设置了PROPAGATE状态，那就传播
    private void setHeadAndPropagate(Node node, int propagate) {
        ...
        // 或者原来的头结点不存在或为待通知唤醒状态中
        // 或者头结点不存在或为待通知唤醒状态中(表明在setHead之后，又有新的该调用)
        //则需要进行唤醒
        if (propagate > 0 || h == null || h.waitStatus < 0 ||
                (h = head) == null || h.waitStatus < 0) {
            Node s = node.next;//node为新加的节点，它的next不为空，并且是s模式，表示是新加的节点
            if (s == null || s.isShared())
                doReleaseShared();//释放后继读锁的关键方法
        }
    }
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {//如果前一个节点取消竞争锁了
             ...
             */
            //将pre节点的状态设置为待通知,此时还可以进行一次尝试,因为在这个过程中前面的节点还可能释放锁
    ...
    private void doAcquireShared(int arg) {
        ...
        try {
            ...
            for (;;) {
                final Node p = node.predecessor();
                if (p == head) {//如果是头节点的下一个节点，也就是当前执行节点的下一个节点，可能在这个过程中，前面的x锁释放
    ...
    final boolean isOnSyncQueue(Node node) {
        if (node.waitStatus == Node.CONDITION || node.prev == null) //如果等待状态为条件通知，或者前一个节点为null，表示不可能在 head->tail队列中
            return false;
        //如果next不为空，则在等待队列中
    ...
    final boolean transferForSignal(Node node) {
         ...
         */
        Node p = enq(node);//获取一个节点
        int ws = p.waitStatus;
        //如果该节点已经取消竞争，或者成功被设置成被通知状态，都进行unpack
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private int checkInterruptWhileWaiting(Node node) {
            return Thread.interrupted() ?
                    (transferAfterCancelledWait(node)//是否是自己加入到等待队列中的
        ...
        public final void await() throws InterruptedException {
            //在await之前会先通过lock获取锁，也就是没有其他资源占有锁时
            if (Thread.interrupted())
                throw new InterruptedException();
            Node node = addConditionWaiter(); //增加等待节点
            int savedState = fullyRelease(node);
            //进行一次release，因为此时其他地方也可能在尝试获取锁，这里让他们成功
            int interruptMode = 0;
            while (!isOnSyncQueue(node)) { //如果不在待通知队列中
                LockSupport.park(this);//则等待
                if ((interruptMode = checkInterruptWhileWaiting(node)) != 0)
                    break;
            }
            //此时node在等待队列中
            //下面走获取锁流程获得一个锁，在await之后再unlock掉
            if (acquireQueued(node, savedState) && interruptMode != THROW_IE)
                interruptMode = REINTERRUPT;
            //如果acquireQueued结果表示当前线程需要interrupt,同时是当前线程加入线程加入到等待队列中到，则interruptMode仍为THROW_IE
